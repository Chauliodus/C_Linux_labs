...
int main(int argc, char *argv[])
{
    ...
    while(1) {
		...
		if( strcmp( msg->send_str, "Есть сообщения" ) == 0 ) {
			connSock = Socket(AF_UNIX, SOCK_STREAM, 0);
			Connect(connSock, (struct sockaddr *) &connectServAddr,
								sizeof(connectServAddr));
			char cmd[3] = "str";
			char bla[25];
			if(send(connSock, cmd, sizeof(cmd), 0) < sizeof(cmd)) DieWithError("cli_2: send() failed");
			recv(connSock, bla, 25, 0);
			printf("От клиента получено %s\n", bla);
			sleep(2);
		}
	}
